    ...
    <div class="max-w-7xl mx-auto px-4">
      ...
      <div class="text-center mb-12">
        ...
        <p class="text-xl text-gray-600 font-light max-w-3xl mx-auto">
          Découvrez notre sélection exclusive de montres de prestige en dépôt-vente, 
          authentifiées et garanties par nos experts horlogers
        </p>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg p-6 mb-12">
        <div class="flex flex-wrap items-center justify-between gap-4">
          ...
          <div class="text-sm text-gray-600 font-light">
            {{ filteredWatches.length }} montre{{ filteredWatches.length > 1 ? 's' : '' }} disponible{{ filteredWatches.length > 1 ? 's' : '' }}
          </div>
        </div>
      </div>
...
const watches = ref([
  {
    ...
    details: {
      ...
      caseCondition: 'Excellent état',
      dialCondition: 'Excellent état',
      braceletCondition: 'Excellent état',
      accessories: [
    ...
    description: 'Rolex Submariner Date iconique en excellent état, parfaite pour la plongée et l\'usage quotidien.'
  },
  // ... (autres montres avec détails similaires)
])

// Navigation method
const handleViewDetails = (watchId) => {
  // In a real Vue Router app, you would use:
  // this.$router.push(`/watch/${watchId}`)
